...
protected:
  ...
  // boundary around the storage api.
  ...
  auto handleCachePromise(jsg::Lock& js, kj::Promise<T> promise, Func&& func,
    ...
    if (allowConcurrency) {
      return context.awaitIo(js, kj::mv(promise), kj::fwd<Func>(func));
    } else {
      return context.awaitIoWithInputLock(js, kj::mv(promise), kj::fwd<Func>(func));
    }
  }
  template <typename T, typename Options, typename Func>
  auto handleCachePromise(jsg::Lock& js, kj::Promise<T> promise, const Options& options,
                          Func&& func) {
    return handleCachePromise(js, kj::mv(promise), kj::fwd<Func>(func),
                              options.allowConcurrency.orDefault(false));
  }
  ...
  auto handleCacheResult(jsg::Lock& js, kj::OneOf<T, kj::Promise<T>> result, const Options& options,
    ...
    KJ_SWITCH_ONEOF(result) {
      ...
      KJ_CASE_ONEOF(promise, kj::Promise<T>) {
        auto wrapper = [func = kj::fwd<Func>(func)](jsg::Lock& js, T&& value) mutable {
          return func(js, kj::mv(value), WasCached::UNCACHED);
        };
        return handleCachePromise(js, kj::mv(promise), options, kj::mv(wrapper));
      }
    }
    KJ_UNREACHABLE;
  }
  ...
  auto handleCacheBackpressure(
    ...
    KJ_IF_SOME(backpressure, maybeBackpressure) {
      return handleCachePromise(js, kj::mv(backpressure), options, [](jsg::Lock&){});
    } else {
